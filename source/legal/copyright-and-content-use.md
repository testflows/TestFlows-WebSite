---
title: "TestFlows Website: Copyright and Content Use"
description: Copyright, trademarks, and permitted use of testflows.com content.
layout: legal
legal_family: website
permalink: legal/copyright-and-content-use/index.html
---

# Copyright and Content Use

<p class="legal-updated">Last updated: 2 August 2026</p>

© 2026 Katteli Inc. All rights reserved.

This notice explains who owns the content on the testflows.com website (the
“**Site**”) and how you may use it. It covers the Site and its content. The
open-source TestFlows testing framework is licensed under its own open-source
license in the project source repository, and that license, not this notice,
governs the framework’s source code.

## 1. Ownership

The Site content, including documentation, the handbook, blog posts, images, and
page design, is owned by Katteli Inc. and is protected by copyright
and other laws.

## 2. What you may do

You may read the content for personal and internal business reference, quote
reasonable excerpts with attribution to TestFlows and a link back to the source
page, and link to any page on the Site.

## 3. What you may not do

Without our written permission you may not republish or mirror substantial
portions of the content, translate it or otherwise create derivative works from
it, sell or otherwise commercially exploit it, remove any copyright or
attribution notice, or present it in a way that implies an endorsement or
affiliation that does not exist.

## 4. Trademarks

“**TestFlows™**,” “**TestFlows Machine™**,” and the TestFlows logo are
trademarks of Katteli Inc. You may use them only to refer accurately to our
products, and never in a way that implies sponsorship or endorsement.

## 5. Copyright concerns

If you believe content on the Site infringes your copyright, please email
`legal@testflows.com` with enough detail to identify the work and the material
at issue, and we will review it.

## 6. Contact

Questions about content use or permissions?

Katteli Inc.

Legal: `legal@testflows.com`  
Website: `https://testflows.com`
