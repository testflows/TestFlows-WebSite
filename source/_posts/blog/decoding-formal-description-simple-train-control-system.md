...
description: An article about how to read formal mathematical description to test a simple train control system.
date: 2024-10-30
...
2. **Initial States** ({%katex%} S_0 {%endkatex%}): A subset of {%katex%} S {%endkatex%} that contains the states in which the system can start. Initial states provide the starting point(s) for evaluating the system's behavior.
...
Second, we can design the **Initial States** ({%katex%} S_0 {%endkatex%}) for the controller to be any state where {%katex%}\ell = \text{NS}{%endkatex%} (internal state is in Normal Status), {%katex%}W = 0{%endkatex%} (warning signal is off), and {%katex%}EB = 0{%endkatex%} (emergency braking is off).
...
- **Condition** {%katex%} \mid \ell = \text{NS}, W = 0, EB = 0 {%endkatex%}: This part specifies constraints for the elements in {%katex%} S_0 {%endkatex%}. It states that:
  - {%katex%} \ell {%endkatex%} must be equal to **NS** (Normal Status),
...
The transition relation {%katex%} R {%endkatex%} is one of the most important parts of the system description, providing a set of rules that describe how the system shifts between states. In the paper, this transition relation is "specified by the predicate."

> "The transition relation {%katex%}R{%endkatex%} is specified by the predicate."

## What does "specified by a predicate" mean?
...
- {%katex%}s = (V_{\text{est}}, V_{\text{MRSP}}, \ell, W, EB){%endkatex%} and {%katex%}s' = (V'_{\text{est}}, V'_{\text{MRSP}}, \ell', W', EB'){%endkatex%} represent the current and next states, respectively.
- The **triple equal sign** {%katex%}\equiv{%endkatex%} in formal logic and mathematics indicates "definition" or "is defined as," meaning that the left side is not just equal to the right side but is defined by it.

The expression {%katex%} \bigvee_{i=0}^{7} \varphi_i {%endkatex%} represents a **disjunction** (logical OR) of multiple conditions, indexed from {%katex%} i = 0 {%endkatex%} to {%katex%} i = 7 {%endkatex%}. 
...
Thus, {%katex%} \bigvee_{i=0}^{7} \varphi_i {%endkatex%} means that the expression is true if at least one of the predicates {%katex%} \varphi_0, \varphi_1, \dots, \varphi_7 {%endkatex%} is true. We can make this more explicit by rewriting it as:
...
As we can see from the above descriptions, the math describes the correct behavior much more precisely and concisely compared to natural language. As intimidating and confusing as it may seem at first glance, once you get used to it, you may start to appreciate the elegance of formal mathematics.

# The labeling function

With transition relations broken down into pieces, we now move on to the next element in the Kripke structure—the labeling function ({%katex%} L {%endkatex%}). We know that the labeling function ({%katex%} L {%endkatex%}) maps each state to a set of atomic propositions that are true in that state. So, the key here is to identify the set of atomic propositions; then this function will tell us which of those propositions are true in a given state.
...
As expected, this formula describes the **labeling function** {%katex%}L{%endkatex%}, which assigns a set of atomic propositions that are true in each state. Here’s how to interpret it:

- The **upside-down A symbol** ({%katex%} \forall {%endkatex%}) is a mathematical symbol meaning "for all" or "for every."

- The **element of symbol** ({%katex%} \in {%endkatex%}) denotes "belongs to" or "is an element of."

- **{%katex%}\forall s \in S{%endkatex%}**: This means "for all {%katex%}s{%endkatex%} in {%katex%}S{%endkatex%}." {%katex%}S{%endkatex%} is the set of all states in the system, indicating that the labeling function {%katex%}L{%endkatex%} is defined for each state {%katex%}s{%endkatex%} in {%katex%}S{%endkatex%}.
...
- **{%katex%}\{ p \in AP \mid s(p) \}{%endkatex%}**: This is **set-builder notation**. This set represents all atomic propositions {%katex%}p{%endkatex%} in {%katex%}AP{%endkatex%} for which {%katex%}s(p){%endkatex%} is true. Here:
  - **{%katex%}p \in AP{%endkatex%}**: {%katex%}p{%endkatex%} is an atomic proposition that belongs to the set {%katex%}AP{%endkatex%}.
  - **{%katex%}s(p){%endkatex%}**: This notation implies that {%katex%}p{%endkatex%} holds (is true) in the state {%katex%}s{%endkatex%}. {%katex%}s(p){%endkatex%} alone implies that {%katex%}p{%endkatex%} is true in the given context. This shorthand keeps the math concise and avoids redundancy.

Breaking down the formula shows that for each state {%katex%}s{%endkatex%} in the system, the labeling function {%katex%}L(s){%endkatex%} returns the set of all atomic propositions {%katex%}p{%endkatex%} that are true in {%katex%}s{%endkatex%}. This allows us to understand which properties or conditions hold in each state of the Kripke structure describing the system.

Since the labeling function depends on atomic propositions, let’s move on to define the atomic propositions for the **Ceiling Speed Monitoring Controller**.

# The atomic propositions

We've finally reached the **atomic propositions** grouped together in the set {%katex%}AP{%endkatex%}. Let’s define what these atomic propositions are and what they represent. So, what exactly is an atomic proposition?
...
1. **Atomic**:  
   The word "atomic" comes from the Greek word "atomos," meaning "indivisible." In system modeling, "atomic" means that the proposition is a fundamental, indivisible statement that cannot be broken down into smaller logical components.  
...
Therefore, an **atomic proposition** is a simple, indivisible statement that cannot be broken down further and thus cannot contain other logical operators such as {%katex%}\land{%endkatex%} (AND) or {%katex%}\lor{%endkatex%} (OR). It represents a specific condition in the system that can be true or false.

Knowing the atomic propositions is critical for testing, and in an ideal scenario, this would be the first question a tester would ask before testing any system: What is the complete set of **atomic propositions** for the system? Knowing **atomic propositions** is crucial because, from them, you can build **equivalence classes** that can provide exhaustive test coverage if the list of atomic propositions is exhaustive and matches those in the actual implementation.

This is the core point of the [*Exhaustive Model-Based Equivalence Class Testing* by W. Huang and J. Peleska, 2013](https://link.springer.com/chapter/10.1007/978-3-642-41707-8_4) paper, which demonstrates that under certain conditions, exhaustive testing is achievable through equivalence classes if the model of the system is known. Here is a quote from the abstract:
...
For the **Ceiling Speed Monitoring Controller**, the paper specifies the complete set of atomic propositions as:
...
We also need the definition of both guard conditions:
...
If we go through each {%katex%}\varphi_i{%endkatex%}, we can list all the atomic propositions (conditions). Let’s go from top to bottom. For each {%katex%}\varphi_i{%endkatex%}, we’ll go from left to right and identify all unique propositions. 

Note that while it might not be intuitive, statements such as {%katex%}\ell = \text{NS}{%endkatex%} are propositions, as these statements can evaluate to either true or false. Why? Because {%katex%}\ell{%endkatex%} can either equal {%katex%}\text{NS}{%endkatex%} or not. Each proposition will include one of the variables that are part of the state, which we know is described by a tuple containing all input, model (internal), and output variables. Given that {%katex%}s = (V_{\text{est}}, V_{\text{MRSP}}, \ell, W, EB){%endkatex%}, we are looking for propositions related to the {%katex%}V_{\text{est}}, V_{\text{MRSP}}, \ell, W, EB{%endkatex%} variables.
...
We skip {%katex%}W' = W{%endkatex%} and {%katex%}EB' = EB{%endkatex%} because these propositions say that these variables will not change and nothing about the actual values of these variables. However, we know that the initial state has these variables set to {%katex%}W = 0, EB = 0{%endkatex%}, which means that the warning signal is off and emergency braking is off as well. Thus, we can add them as:
...
Note we don’t repeat atomic propositions that we’ve already selected. We only look for new ones. Therefore, since we already have {%katex%}\ell = \text{NS}{%endkatex%}, we don’t need to add it again.

From {%katex%}\varphi_2: (\ell = \text{NS} \land (g_{\text{ebi1}} \lor g_{\text{ebi2}}) \land \ell' = \text{IS} \land W' = 1 \land EB' = 1) {%endkatex%} we get:

> {%katex%}
V_{\text{MRSP}} > 110, V_{\text{est}} > V_{\text{MRSP}} + 15, V_{\text{MRSP}} \leq 110, V_{\text{est}} > V_{\text{MRSP}} + 7.5, \ell = \text{IS}, EB = 1
{%endkatex%}

Remember, because propositions are atomic, we broke down the definitions of {%katex%}g_{\text{ebi1}}{%endkatex%} and {%katex%}g_{\text{ebi2}}{%endkatex%} into their atomic parts:
{%katex%}
g_{\text{ebi1}} \equiv V_{\text{MRSP}} > 110 \land V_{\text{est}} > V_{\text{MRSP}} + 15 \\
g_{\text{ebi2}} \equiv V_{\text{MRSP}} \leq 110 \land V_{\text{est}} > V_{\text{MRSP}} + 7.5
{%endkatex%}<br><br>
...
This is because we already have {%katex%}\ell = \text{WS}{%endkatex%} and {%katex%}V_{\text{est}} > V_{\text{MRSP}}{%endkatex%}. The expression {%katex%}\neg(g_{\text{ebi1}} \lor g_{\text{ebi2}}) = \neg(g_{\text{ebi1}}) \land  \neg(g_{\text{ebi2}}){%endkatex%} can be derived using [De Morgan's Law](https://en.wikipedia.org/wiki/De_Morgan's_laws).
...
From {%katex%}\varphi_5: (\ell = \text{WS} \land (g_{\text{ebi1}} \lor g_{\text{ebi2}}) \land \ell' = \text{IS} \land W' = W \land EB' = 1) {%endkatex%}, we also don't get anything new.
...
Why? Because our set includes redundant propositions. **Redundant propositions are those that are simply the negation of another proposition.** So, let's go through our list and identify all the redundancies to match the expected result above.

Upon careful observation, we find the following redundant propositions:
- {%katex%}V_{\text{est}} > 0{%endkatex%} is redundant because {%katex%}\neg(V_{\text{est}} = 0) = (V_{\text{est}} > 0) \land (V_{\text{est}} < 0) {%endkatex%}. However, {%katex%}V_{\text{est}} < 0{%endkatex%} does not make sense, as the estimated speed cannot be negative, so we are left with {%katex%}V_{\text{est}} > 0{%endkatex%}. Therefore, we don’t need it.
...
- {%katex%}W = 0{%endkatex%} is just {%katex%}\neg(W = 1){%endkatex%}, which we can simply write as {%katex%}W{%endkatex%}.
- {%katex%}EB = 0{%endkatex%} is just {%katex%}\neg(EB = 1){%endkatex%}, which we can simply write as {%katex%}EB{%endkatex%}.

Therefore, we remove the redundancies from our set as follows:
...
After removing all the crossed-out redundant propositions and rearranging the elements, we do get the set of atomic propositions as stated in the paper.
...
We've successfully decoded the formal description of a simple train control system, the **Ceiling Speed Monitoring Controller**, as described in [*Exhaustive Model-Based Equivalence Class Testing* by W. Huang and J. Peleska, 2013](https://link.springer.com/chapter/10.1007/978-3-642-41707-8_4). In the paper, the system was formally described using a Kripke structure. We explored what a Kripke structure is, examined each component used to define it, and saw how it applies to the controller. 

We closely analyzed how a transition relation can be specified as a predicate—a mathematical formula—and from it, we extracted a set of atomic propositions from which equivalence classes can be derived to implement test cases. When a complete list of atomic propositions is known for a given implementation, the set of test cases constructed from equivalence classes can result in an exhaustive test suite, which is challenging to achieve without the understanding and rigor provided by formal mathematics.

While tackling formal definitions may not be easy at first, I hope this article, where I tried to explain the math in as much detail as possible, will inspire more test engineers not to be intimidated by it. Formal concepts provide a strong foundation for thinking about systems abstractly, a critical skill needed when testing real-world software systems.
