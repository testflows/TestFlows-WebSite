...
description: An article about how to read formal mathematical description to test simple train control system.
date: 2024-10-30
...
In scientific research on software testing, you come across papers filled with complex mathematical descriptions. Formal mathematics is commonly used in academia because it offers a precise way to describe how a system should behave in various situations, helping researchers ensure that all possible cases are accounted for. But for many testers, the math in these papers can feel overwhelming and difficult to apply to everyday testing work.
...
In this post, we’re going to break down an example presented in [*Exhaustive Model-Based Equivalence Class Testing* by W. Huang and J. Peleska, 2013](https://link.springer.com/chapter/10.1007/978-3-642-41707-8_4). This paper provides a formal description of a ceiling speed monitoring function for a train. We’ll walk through the process of interpreting this formal description, the [Kripke structure](https://en.wikipedia.org/wiki/Kripke_structure_(model_checking)), learn to understand each element of it including states, initial states, the transition relation, the labeling function and most importantly the atomic propositions from
which equivalence classes can be build to provide under certain conditions an exhaustive test suite - the ultimate dream for any tester.
...
<div class="text-secondary text-bold"><br>Diagram: Celing Speed Monitoring I/O Interface</div>
</div><br>
...
<div class="text-secondary text-bold"><br>Diagram: Celing Speed Monitoring State Diagram</div>
</div><br>
...
Here is the description of each state and thresholds:
...
In the paper, the above system is formally described by a [Kripke structure](https://en.wikipedia.org/wiki/Kripke_structure_(model_checking)).
While at first glance it might sound and look intimidating we can learn to 
...
4. **Labeling Function** ({%katex%} L {%endkatex%}): A function that maps each state to a set of atomic propositions that are true in that state. Atomic propositions describe properties or facts about the system (e.g., “speed exceeds limit” or “warning signal is on”). The labeling function helps define the truth values of these propositions within each state.

5. **Atomic Propositions** ({%katex%} AP {%endkatex%}): The set of all **atomic propositions**. Atomic propositions are simple statements about the system that can be either true or false in each state. They describe specific properties or conditions (e.g., “speed exceeds limit” or “emergency brake is active”) that help define what’s happening in any given state. The labeling function {%katex%} L {%endkatex%} uses these atomic propositions to indicate which conditions hold true in each state, making it possible to analyze the system’s behavior based on these fundamental truths.

In summary, a Kripke structure captures the dynamics of a system by defining its states, how it can transition between states, and which conditions hold true in each state. This allows for systematic analysis of the system’s behavior and helps in verifying that it meets system's requirements or avoids unwanted conditions.

Let's start unraveling what each element in the Kripke structure is as
it applies to the **Ceiling Speed Monitoring Controller**.

# The states

First, **States** ({%katex%} S {%endkatex%}) is set of all possible states.
But, what does the actual **State** looks like? The **State** consists of
**input**, **internal (model)**, and **output** variables. In general,
it could also include even **environment** variables. It is critical to grasp
that **input** and **output** variables are also part of the state!
In some cases people think that states of the system are only the circles
that you see in the state diagram. This is not the case.
It is just a part of the overall state description, in this case the circles in the state diagram are represented as {%katex%}\ell {%endkatex%}, where {%katex%}\ell {%endkatex%} is an internal variable that can have values {%katex%}\text{NS}, \text{WS}, \text{IS}{%endkatex%} corresponding to each status. So after including **input**, **internal (model)**, and **output** variables in
the state for the **Ceiling Speed Monitoring Controller**, its state is represented as a tuple:
...
to be any state where {%katex%}l=NS{%endkatex%} (internal state is in Normal Status), {%katex%}W=0{%endkatex%} (warning signal is off), {%katex%}EB=0{%endkatex%} (emergency braking is off). 

This can be expressed mathematically as:

> {%katex%}S_0=\{(V_{est},V_{MRSP},l,W,EB)|l=NS,W=0,EB=0\} {%endkatex%}. 
...
- **Condition** {%katex%} \mid \ell = \text{NS}, W = 0, EB = 0 {%endkatex%}: This part specifies constraints for the elements in {%katex%} S_0 {%endkatex%}. It states that:
  - {%katex%} \ell {%endkatex%} must be equal to
    **NS** (Normal Status),
  - {%katex%} W {%endkatex%} must be
    `0` (warning signal is off),
  - {%katex%} EB {%endkatex%} must be
    `0` (emergency brake is inactive).
...
The transition relation {%katex%} R {%endkatex%} is one of the most important parts of the system description and provides a set of rules describing how the system shifts between states. In the paper, this transition relation is "speciﬁed by the predicate."
...
For the **Ceiling Speed Monitoring Controller**, {%katex%} R {%endkatex%} is defined as:
...
Where the {%katex%}s = (V_{\text{est}}, V_{\text{MRSP}}, \ell, W, EB){%endkatex%}
...
The scary looking {%katex%} \bigvee_{i=0}^{7} \varphi_i {%endkatex%} represents a **disjunction** (logical OR) of multiple conditions, indexed from {%katex%} i = 0 {%endkatex%} to {%katex%} i = 7 {%endkatex%}. 
...
Thus, {%katex%} \bigvee_{i=0}^{7} \varphi_i {%endkatex%} means that the expression is true if at least one of the predicates {%katex%} \varphi_0, \varphi_1, \dots, \varphi_7 {%endkatex%} is true. We can be more explicit and re-write it as:
...
We can substitute the definitions {%katex%} \varphi_0, \varphi_1, \dots, \varphi_7 {%endkatex%} into {%katex%}R{%endkatex%} to get the final
transition relation that fully defines how internal (model) variable {%katex%}\ell{%endkatex%} and output variables {%katex%}W, EB{%endkatex%} change in response
to the current status and input variables.

We get something that at first glass is hard to understand. However, be patient
as we'll break down what all of this means.
...
💡 It is important to note that the above expression defines a **formula** that describes the behavior of the system. This **formula** must hold for the system to operate correctly. Therefore, transition relations in finite-state systems can indeed be represented as **mathematical formulas**! This is very powerful and shows that
mathematics can model complex systems.
...
As we can see from the above descriptions, the math describes the correct behavior
much more precisely and concisely as compared to natural language. As scary
that confusing it might seem at the first glance, once you get used to it
you find yourself to start preferring the elegance of formal mathematics as well.

# The labeling function

With transition relations broken down into pieces, we now move on to the next
element in the Kripke structure - the labeling function ({%katex%} L {%endkatex%}).
We know that the the labeling function ({%katex%} L {%endkatex%}) maps each state to a set of atomic propositions that are true in that state. So the key here
is to know what the set of atomic propositions are and then this function
will tell us which of those propositions will be true at a given state.
...
We've finally reach the **atomic propositions** which are grouped together in the set {%katex%}AP{%endkatex%}. But let's define what these atomic propositions are and what do they represent. What is atomic proposition?

The term **atomic proposition** is commonly used in formal logic and system modeling to describe the basic, indivisible statements that can be true or false in a given state of a system. It can be thought of as a **basic condition**. Here’s a breakdown of each word:

1. **Atomic**:
   The word "atomic" comes from the Greek word "atomos," meaning "indivisible."
   For system modeling, "atomic" means that the proposition is a fundamental, indivisible statement that cannot be broken down into smaller logical components.
   For example, "the train’s estimated speed is greater than the maximum allowed speed" is an atomic statement because it cannot be further simplified in the model.

2. **Proposition**:
   In logic, a proposition is a declarative statement that is either true or false.
   In the context of system modeling, a proposition represents a specific condition or property that can hold in a given state. For example, "emergency brake is active" could be a proposition that is true if the brake is on and false if it is off.

Therefore, an **atomic proposition** is a simple, indivisible statement, cannot be broken down further and therefore can't contain other logical operators such as {%katex%}\land{%endkatex%} (AND) or {%katex%}\lor{%endkatex%} (OR), about the specific condition in the system that can be true or false.

Knowing the atomic propositions is critical for testing and
in the ideal world this would be the first question a tester would ask before
testing any system. What is the complete set of **atomic propositions** for the system?
The reason why knowing **atomic propositions** is critical to testing, is because
from them you can build **equivalence classes** that can provide exhaustive test coverage if the list of the atomic propositions is itself exhaustive and matches the atomic propositions of the actual implementation.

This is the whole point of the [*Exhaustive Model-Based Equivalence Class Testing* by W. Huang and J. Peleska, 2013](https://link.springer.com/chapter/10.1007/978-3-642-41707-8_4) paper, which shows that under certain conditions exhaustive testing
is possible to achieve using equivalence classes if the model of the system is known.
Here is the quote from the abstract:

> "We prove that a finite collection of input traces whose elements have been selected from a specific set of input equivalence classes suffices to prove a conformance relation between specification model and system under test."
...
But how do you build this list, or more precisely, this set? The answer is simple. You can get it
from the the transition relation {%katex%}R{%endkatex%} specified as a predicate.
Why? Because a **predicate** is a statement or function that contains variables and becomes a **proposition** when specific values are assigned to those variables.
...
Here is the {%katex%}R{%endkatex%} again from which we can pull out the propositions:
...
We also need the definition of both of the guard conditions:
...
If go through each {%katex%}\varphi {%endkatex%} we can list all the atomic propositions (conditions). Let's go from the top to bottom. For each {%katex%}\varphi {%endkatex%}, we'll go from left to right and identify all propositions
that are unique. Note that it might not be intuitive but statements such as {%katex%}\ell = \text{NS}{%endkatex%} is a proposition and this statement can evaluate to
either true or false. How come? Becuase {%katex%}\ell{%endkatex%} can be equal to the value {%katex%}\text{NS}{%endkatex%} or not. Each proposition will include one of the variables that are part of the state, which we know is described by a tuple that contains all input, model (internal), and output variables. Given that {%katex%}s = (V_{\text{est}}, V_{\text{MRSP}}, \ell, W, EB){%endkatex%}, therefore we are looking for propositions related to {%katex%}V_{\text{est}}, V_{\text{MRSP}}, \ell, W, EB{%endkatex%} variables.
...
We skip {%katex%}W' = W{%endkatex%} and {%katex%}EB' = EB{%endkatex%} because
these propositions say that these variables will not change and nothing about what
the actual values of these variables are. However, we know that initial state has these
variables set to {%katex%}W = 0, EB = 0{%endkatex%} which means that the warning signal is off and emergency braking is off as well. Thus, we can add them as:
...
Note we don't repeat atomic propositions that we've already picked. We only looking
for the new ones. Therefore, because we already have {%katex%}\ell = \text{NS}{%endkatex%}, so we don't need to add it
...
Remember because propositions are atomic we took the definitions of {%katex%}g_{\text{ebi1}}{%endkatex%} and {%katex%}g_{\text{ebi2}}{%endkatex%} and broken them down into atomic parts.
{%katex%}
g_{\text{ebi1}} \equiv V_{\text{MRSP}} > 110 \land V_{\text{est}} > V_{\text{MRSP}} + 15 \\
g_{\text{ebi2}} \equiv V_{\text{MRSP}} \leq 110 \land V_{\text{est}} > V_{\text{MRSP}} + 7.5
{%endkatex%}<br><br>
...
From {%katex%} \varphi_5: (\ell = \text{WS} \land (g_{\text{ebi1}} \lor g_{\text{ebi2}}) \land \ell' = \text{IS} \land W' = W \land EB' = 1) {%endkatex%} we don't get anything new as well.
...
Why? Because our set includes redundant propositions. **The redundant propositions are the propositions which are just negation of another proposition**. So, let's go through our list and identify all the redundencies to match the expected result above.

Upon careful observation we find the following redundant propositions:
- {%katex%}V_{\text{est}} > 0{%endkatex%} is redundant because {%katex%}\neg(V_{\text{est}} = 0) = (V_{\text{est}} > 0) \land (V_{\text{est}} < 0) {%endkatex%}, but
{%katex%}V_{\text{est}} < 0{%endkatex%} does not make sense because the estimated speed cannot be negative so we are left with {%katex%}V_{\text{est}} > 0{%endkatex%}.
Therefore, we don't need it.
- {%katex%}V_{\text{est}} \leq V_{\text{MRSP}}{%endkatex%} is just {%katex%}\neg(V_{\text{est}} > V_{\text{MRSP}}){%endkatex%}, so we can remove it.
- {%katex%}V_{\text{MRSP}} \leq 110{%endkatex%} is just {%katex%}\neg(V_{\text{MRSP}} > 110){%endkatex%}.
- {%katex%}V_{\text{est}} \leq V_{\text{MRSP}} + 7.5{%endkatex%} is just {%katex%}\neg(V_{\text{est}} > V_{\text{MRSP}} + 7.5){%endkatex%}.
- {%katex%}V_{\text{est}} \leq V_{\text{MRSP}} + 15{%endkatex%} is just {%katex%}\neg(V_{\text{est}} > V_{\text{MRSP}} + 15){%endkatex%}.
- {%katex%}W = 0{%endkatex%} is just {%katex%}\neg(W = 1){%endkatex%} which we can just write as {%katex%}W{%endkatex%}.
- {%katex%}EB = 0{%endkatex%} is just {%katex%}\neg(EB = 1){%endkatex%} which we can just write as {%katex%}EB{%endkatex%}.

Therefore, we remove the redundencies from our set as follows:
...
\quad V_{\text{est}} > V_{\text{MRSP}}, \ell = \text{WS}, W = 1, \\
\quad V_{\text{MRSP}} > 110, V_{\text{est}} > V_{\text{MRSP}} + 15,
\cancel{V_{\text{MRSP}} \leq 110}, V_{\text{est}} > V_{\text{MRSP}} + 7.5, \ell = \text{IS}, EB = 1, \\
...
After removing all the crossed out redundant propositions and rearranging the elements we do get the set of atomic propositions as was stated in the paper.
...
With the set of atomic propositions in hand, we can easily create equivalence classes
that can be used in testing as described in [Using Atomic Propositions and Equivalence Classes (Part 1)](..using-atomic-propositions-and-equivalence-classes-part1/) and [Using Atomic Propositions and Equivalence Classes (Part 2)](..using-atomic-propositions-and-equivalence-classes-part2/).

# Conclusion

We've successfully decoded formal description of a simple train control system, the **Ceiling Speed Monitoring Controller**, described in [*Exhaustive Model-Based Equivalence Class Testing* by W. Huang and J. Peleska, 2013](https://link.springer.com/chapter/10.1007/978-3-642-41707-8_4). In the paper, the system was formally described using the Kripke structure. We've learned what it is and looked at each component used to define it and saw how it applied to the controller. We've closely analyzed
how a transition relation can be specified as a predicate, a mathematical formula, and how from it we can extract a set of atomic propositions from which equivalence classes
can be derived to implement test cases. When a complete list of atomic propositions
is known for a given implementation, the set of test cases constructed from equivalence classes can result in exhaustive test suite which is hard to achieve
without understanding and rigor of formal mathematics. While tackling formal definitions is not easy at first, I hope this article, where I tried to explain the math in as much detail as possible, will inspire more test engineers to not be afraid of it, because the formal concepts provide great foundation for thinking about systems that we test abstractly which is critical skill that is required during testing real-world software systems.