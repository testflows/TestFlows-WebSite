...
*"Requirements are the starting point of any design"*, this is a quote from the [presentation given by Nicholas D. Kefalas](https://www.youtube.com/watch?v=3afHlAwBudM), who is a Senior Technical Officer at Sikorsky Aircraft - Lockheed Martin. Very innocent quote that most people
...
Let's take the liberty of redefining these terms close to how {% testflows %} uses them to fully implement enterprise level software quality assurance process. First of all, a tester for me at the beginning of my career was not a person but a multi-million,
...
How can we work with requirements that matches software development as close as possible? Simple, work with requirements just like with code.
Before we can work with requirements just like with code we need to understand two core concepts.

First, requirements must be written down. Sounds obvious but most of the software requirements are not written down. Most of the requirements sit in someone’s head and are never written down. Therefore, you constantly have to ask questions and you constantly forget part of the functionality. It is hard for us to keep all the details of even simple programs in our head, so it is impossible to keep track of all of the functionality for any non trivial project.

Second, written down requirements are grouped into software requirements specification (SRS) document. Have you seen a software requirements
...
Now, the most important question, who writes the software requirements specification documents? Well, at the end quality assurance team ends up writing
them given that it usually not done by other teams. Again remember, *"requirements are the starting point of any design"*, yeah, but in most
...
in GitHub or GitLab. We get full traceability of origin and modifications and it naturally ties in with automated test development. Add or modify a requirement, then commit. Diff, blame, history, you get it all for free!
...
Let’s look at a simple example. You know Google and you know that if you type “calculator” string in the search box you will get a calculator web application? Funny enough it is actually not documented, at least I could not find any official documentation for it. Who needs it?
...
Let’s see how a software requirements specification for a Google calculator can look like.
...
There is a lot of similarity with user documentation but requirements are
usually much more detailed and meet their own requirements like being unique, having a unique identifier, version, use SHALL, and being testable.
...
First thing to note is that each requirement has a unique identifier.
...
The initial version of a requirement by convention is *1.0*. Any material changes to a requirement would require us to increment
...
Instead the dot notation is used. The dot notation also defines hierarchy. Using dot notation instead of number facilitates moving requirements
...
The only special thing to worry about is to remember that to define a requirement you must define a heading that starts with **RQ** and give it a version.
...
Do we really want to keep this only in our head or is it easier to write this down once and then reference it when needed?
...
When writing tests we can import requirement objects as needed, and the IDE will
...
becomes professional and easy. Also, it is important to note that quality assurance team does not need all of the requirements at once, we just need one!
